...
Casas.rf <- randomForest( median_house_value ~., data = ModelTrain, ntree = 1000, mtry = 5, importance = TRUE, keep.forest = TRUE)
```
...
```
```{r}
ModelTest <- TestSUMedian %>%
  dplyr :: select(-id)
```
```{r}
ids <- TestSUMedian$id
prediccion <- predict(object = Casas.rf, newdata = TestSUMedian)
